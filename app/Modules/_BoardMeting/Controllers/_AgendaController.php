...
class AgendaController extends Controller
{
    ...
    public function getAgendaData(request $request)
    {
        ...
        return Datatables::of($agendaList)
            ->addColumn('action', function ($agendaList) use ($mode,$boardMeetingStatus) {
                if ($mode) {
                    ...
                    if (!in_array($boardMeetingStatus->status, [5, 10]) && $userType =='13x303') {  //5= fixed status 10=complete
...
left join (select GROUP_CONCAT(process_list.tracking_no ,\"AAAAA\", process_list.json_object SEPARATOR \"@@\") as pbmdatas,pbm.agenda_id pbmagendaId from  process_list_board_meeting pbm 
left join agenda on pbm.agenda_id = agenda.id 
left join process_list on process_list.id = pbm.process_id
where agenda.board_meting_id = $meeting_id) as agenda_process
on agenda_process.pbmagendaId=agenda.id
where bm.id=$meeting_id";
           $meetingInfo = \DB::select(DB::raw($sql));
//dd($meetingInfo);

           $contents = view('BoardMeting::agenda.agenda-download',compact("meetingInfo"))->render();
//return $contents;
//dd();
        ...
        $mpdf = new mPDF([
           ...
           $certificateName = uniqid("board_meeting_agenda" . 11 . "_", true);
...
//dd($meetingInfo);
            ...
            $contents = view('BoardMeting::agenda.meeting-minutes',compact("meetingInfo"))->render();
            $defaultConfig = (new \Mpdf\Config\ConfigVariables())->getDefaults(); // extendable default Configs
            $fontDirs = $defaultConfig['fontDir'];
    ...
    public function createNewAgenda($board_meeting_id)
    {
        ...
        $process_type = ['' => 'Select One'] + ProcessType::where('status', 1)
                ...
                ->lists('name', 'id')->all();
        return view('BoardMeting::agenda.create-agenda', compact('process_type','board_meeting_data'));
    }
...
//                    $file->agenda_id = $agenda_id->id;
//                    $file->board_meting_id = Encryption::decodeId($request->get('board_meting_id'));
//                    $file->save();
//                }
//            }
    ...
    public function editAgenda($id)
    {
        ...
        return view('BoardMeting::agenda.edit-agenda', compact('agendaData', 'id', 'process_type'));
    }
        ...
        return Datatables::of($list)
            ->addColumn('action', function ($list) use ($status, $request,$agenda,$agenda_id) {
                ...
                if(!in_array($boardMeetingStatus->status,[5,10]))  { //5=fixed
                    ...
                    //  } else {
                    //    $html .= '<button value="' . $list->id . '" class="add_to_board btn btn-warning btn-xs"> Add to Board Meeting vvv </button>';
                    //  }
                }
                return $html;
            })
...
//        $NextBoardMeeting = BoardMeting::where('id', $upcomming)->min('meting_date');
        if($upComing != null){
            $NextBoardMeetingDate ="<div class='alert alert-success' style='font-weight: bold'>It will be transfer to the meeting date: ".date("d-M-Y", strtotime($upComing->meting_date))."  </div>";
            $status = true;
        } else {
            $NextBoardMeetingDate = "<div class='alert alert-danger text-bold'style='font-weight: bold'>Roll-over is not possible, Please create a upcoming board meeting</div>";
            $status = false;
        }
