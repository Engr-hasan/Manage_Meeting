...
<script language="javascript">
    ...
    jQuery(function () {
        jQuery('#submitFromPreviewBtn').click(function (e) {
            window.opener.jQuery("#authorized_mobile").removeClass('engOnly error');
            window.opener.document.getElementById("ApplicationForm").setAttribute("target", "_self");
            window.opener.jQuery("#ApplicationForm").submit();
            window.close();
        });
    });
...
//        window.opener.jQuery('#' + id + ' option:[value="' + val + '"]').replaceWith("<option value='" + val + "' selected>" + text + "</option>");
    ...
    window.opener.jQuery(".preview").addClass('hidden');
...
//    jQuery('input[type=hidden]').not("input[type=hidden][name=eiaCertValue]").remove();
    ...
    jQuery('#previewDiv .documentUrl').each(function () {
        jQuery(this).replaceWith("<i class='fa fa-file-pdf-o'></i>");
    });
    ...
    jQuery('#ApplicationForm :input').attr('disabled', true);
    ...
    function CloseMe()
    {
        ...
        window.opener.jQuery("#ApplicationForm-p-4").css({"display": "block"});
        window.opener.jQuery(".last").addClass('current');
        window.opener.jQuery(".last").removeClass('done');
