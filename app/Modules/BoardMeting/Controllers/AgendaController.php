...
//                ->get(['process_list.company_id','process_list.created_at as process_created','apps.*']);
            ...
            $data = Achievement::leftJoin('process_list', 'achievement.id', '=', 'process_list.ref_id')
                ...
                ->get(['process_list.company_id','process_list.tracking_no','process_list.created_at as process_created','achievement.*']);
...
//            $contents = view('BoardMeting::agenda.meeting-minutes',compact("meetingInfo","board_meeting_data","getChairperson"))->render();
            $contents = view::make('BoardMeting::agenda.agenda-download',compact('data',"meetingInfo","board_meeting_data","getChairperson","committeeInfo"))->render();
            ...
            $certificateName = uniqid("Agenda_", true);
            $pdfFilePath = $directoryName . "/" . $certificateName . '.pdf';
            $mpdf->Output($pdfFilePath, 'F'); // Saving pdf *** F for Save only, I for view only.
            $chekexists=BoardMeting::where('id',$meeting_id)->first();
            if($chekexists->meeting_agenda_path == null) {
...
//                ->get(['process_list.company_id','process_list.created_at as process_created','apps.*']);
            ...
            $data = Achievement::leftJoin('process_list', 'achievement.id', '=', 'process_list.ref_id')
                ...
                ->where('process_list_board_meeting.board_meeting_id', $meeting_id)
                ->get(['process_list.company_id','process_list.tracking_no','board_meeting_process_status.status_name',
                    'process_list.created_at as process_created','achievement.*']);
//            dd($data->toArray());
...
//            $contents = view('BoardMeting::agenda.meeting-minutes',compact("meetingInfo","board_meeting_data","getChairperson"))->render();
            $contents = view::make('BoardMeting::agenda.meeting-minutes',compact('data',"meetingInfo","board_meeting_data","getChairperson","committeeInfo"))->render();
            $defaultConfig = (new \Mpdf\Config\ConfigVariables())->getDefaults(); // extendable default Configs
            $fontDirs = $defaultConfig['fontDir'];
            ...
            $mpdf->Output($pdfFilePath, 'F'); // Saving pdf *** F for Save only, I for view only.
            $chekexists=BoardMeting::where('id',$meeting_id)->first();
            if($chekexists->meeting_minutes_path == null) {
                BoardMeting::where('id', $meeting_id)->update([
                    'meeting_minutes_path' =>$pdfFilePath,
                ]);
            }
    ...
    public function generateMeetingMinutesDoc($board_meeting_id){
        ...
        $committeeInfo = Committee::where('board_meeting_id', $meeting_id)->where('user_email','!=', Auth::user()->user_email)
            ->orderBy('type','desc')
            ->get([
                'user_email',
                'user_name',
...
//            $contents = view('BoardMeting::agenda.meeting-minutes',compact("meetingInfo","board_meeting_data","getChairperson"))->render();
        $contents = view::make('BoardMeting::agenda.meeting-minutes',compact('data',"meetingInfo","board_meeting_data",'committeeInfo',"getChairperson"))->render();
...
//            }
    ...
    public function agendaWiseProcess($agendaId)
    {
        ...
        $boarmeetindata=BoardMeting::where('id',$agendaInfo->board_meting_id)
        ->first();
        $boarmeetindata->sequence_no=20;
        $boarmeetindata->save();
