...
namespace App\Modules\LimitRenewal\Controllers;
...
class LimitRenewalController extends Controller
{
    ...
    public function __construct()
    {
        if (Session::has('lang'))
            ...
            $this->process_type_id = 4; // 3 is CoBrandedCard process type
    }
    ...
    public function applicationForm()
    {
        try{
            ...
            $data['appInfo'] = LimitRenewal::orderby('id','DESC')->first();
            $data['SpecificBusinessProposeData']  = CoBrandedBusinessPurposeSave::where('ref_id',$data['appInfo']->id)->get();
        ...
        return view("LimitRenewal::application-form",compact('data'));
    ...
    public function applicationViewEdit($applicationId)
    {
        ...
        try {
            ...
            $process_history = DB::select(DB::raw("select  `process_list_hist`.`desk_id`,`as`.`status_name`,
                    ...
                    "));
            $appInfo = ProcessList::leftJoin('limit_renewal as apps', 'apps.id', '=', 'process_list.ref_id')
                ->leftJoin('user_desk', 'user_desk.id', '=', 'process_list.desk_id')
                ->leftJoin('process_status as ps', function ($join) use ($process_type_id) {
                    $join->on('ps.id', '=', 'process_list.status_id');
                    $join->on('ps.process_type_id', '=', DB::raw($process_type_id));
...
//            $data['memberShipNo'] = MemberShipNumber::where('is_active',1)->where('user_id', $userInfo->id)->first();
            ...
            return view('LimitRenewal::application-form-edit',
...
//            'amount_of_money' => 'required',
        ...
        try {
            ...
            if ($request->get('app_id')) {
                ...
                $appData = LimitRenewal::find($decodedId);
...
//                $processData = ProcessList::firstOrNew(['process_type_id' => $this->process_type_id, 'ref_id' => $appData->id]);
            ...
            $appData->refile_increment= $request->get('refile_increment');
            $appData->save();
...
//            } /* End file uploading */
            ...
            return redirect('limit-renewal/list/'.Encryption::encodeId($this->process_type_id));
        } catch (\Exception $e) {
...
//        $certificate = $this->certificate_gen($appInfo->ref_id);   //Certificate Generation after payment accepted
        $certificate = "";   //Certificate Generation after payment accepted
        $certificate = "/uploads/basiscardrecformebl.pdf";
        LimitRenewal::where('id', $appInfo->ref_id)->update(['certificate' => $certificate]);
...
//                    }
        ...
        $appStatus = ProcessStatus::where('status', 4)->get();
        $statusList = array();
